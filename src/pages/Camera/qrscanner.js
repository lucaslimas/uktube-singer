class e {
  ...
  constructor(a, c, b = e.DEFAULT_CANVAS_SIZE) {
    ...
    this._sourceRect = { x: 0, y: 0, width: b, height: b };
  ...
  destroy() {
    ...
    this.stop();
    this._qrWorker.postMessage({ type: "close" });
  }
  start() {
    ...
    let a = "environment";
    return this._getCameraStream("environment", !0)
      .catch(() => {
        a = "user";
  ...
  static scanImage(a, c = null, b = null, d = null, f = !1, g = !1) {
    let h = !1,
      l = new Promise((l, g) => {
        b ||
          ((b = new Worker(e.WORKER_PATH)),
          (h = !0),
          b.postMessage({ type: "inversionMode", data: "both" }));
        let n, m, k;
        m = (a) => {
          "qrResult" === a.data.type &&
            (b.removeEventListener("message", m),
            b.removeEventListener("error", k),
            clearTimeout(n),
            null !== a.data.data ? l(a.data.data) : g("QR code not found."));
        };
        k = (a) => {
          b.removeEventListener("message", m);
          b.removeEventListener("error", k);
        ...
        e._loadImage(a)
          .then((a) => {
            ...
            b.postMessage({ type: "decode", data: a }, [a.data.buffer]);
          })
          .catch(k);
      });
    c && g && (l = l.catch(() => e.scanImage(a, null, b, d, f)));
    return (l = l.finally(() => {
      h && b.postMessage({ type: "close" });
    }));
  }
  setGrayscaleWeights(a, c, b, d = !0) {
    this._qrWorker.postMessage({
      type: "grayscaleWeights",
      data: { red: a, green: c, blue: b, useIntegerApproximation: d },
    });
  }
  setInversionMode(a) {
    this._qrWorker.postMessage({ type: "inversionMode", data: a });
  }
  _onCanPlay() {
  ...
  _updateSourceRect() {
    let a = Math.round(
      (2 / 3) * Math.min(this.$video.videoWidth, this.$video.videoHeight)
    );
  ...
  _getCameraStream(a, c = !1) {
    let b = [{ width: { min: 1024 } }, { width: { min: 768 } }, {}];
    a && (c && (a = { exact: a }), b.forEach((b) => (b.facingMode = a)));
